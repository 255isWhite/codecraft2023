...
bool Solution::GetFrameInfo(){
    ...
    while(getline(cin,line)){
        ...
        if(0==count){
            ...
            // if(nums_workbench_ == 25){ // special map1
            //     if( count==1+nums_workbench_ || count==nums_workbench_ || count==nums_workbench_-1){
            //         stringstream ss(line);
            //         count++;
            //         continue;
            //     }
...
void Solution::AssignTasks(){
    ...
    for(int i=0;i<4;++i){
        auto r = robots_[i];
        if(current_frame_>7602 && current_frame_<7780){
        ...
        if(r->busy){
            ...
            if(r->workbench_near == workbenches_[r->target_type][r->target_id].idx){
                ...
                if(thing_type == 0){
                    ...
                    } else if(current_frame_>8750) continue;
                    r->Buy();
                    r->busy = false;
